...
export function ActivityLog({ taskId = null, user }) {
  ...
  useEffect(() => {
    const handleBoardUpdate = async () => {
      console.log('From ActivityLog.jsx: Board updated')
  ...
  return (
    <div className='stal_container'>
      {board.activities.filter((activity) => taskId == null || activity.task.id == taskId) // if taskId==null don't filter activities because we should show all of them
        .map((taskActivity) => (
          <div className='stal_row' key={taskActivity.id}>
            <div className='stal-time-user-title-and-status'>
              <span className='stal-time'>{utilService.calcTimePassed(taskActivity)}</span>
              <Avatar.Root className='AvatarRoot'>
                <Avatar.Image
                  className='AvatarImage'
                  src={taskActivity?.byMember ? taskActivity?.byMember?.imgUrl  : 'https://cdn1.monday.com/dapulse_default_photo.png'}
                  alt={taskActivity?.byMember ? taskActivity?.byMember?.fullname : 'User Avatar'}
                />
                <Avatar.Fallback className='AvatarFallback' delayMs={600}>
                  {/* incase avatar cant be loaded show letters */}
                </Avatar.Fallback>
              </Avatar.Root>
              <span>{taskActivity?.byMember?.fullname}</span>
